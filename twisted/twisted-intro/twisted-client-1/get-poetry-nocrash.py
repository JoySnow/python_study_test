...
class PoetrySocket(object):
    ...
    def __init__(self, task_num, address):
        ...
        # THIS IS WHAT JOY DO:
        #   once something wrong with the connect,
        #   let self.sock be None, and don't add it into reactor.
        try:
            self.sock.connect(address)
        except socket.error as msg:
            self.sock.close()
            self.sock = None
            print "Task ", self.task_num, " failed here ..."
            return
...
def poetry_main():
    ...
    from twisted.internet import reactor
    print "Nothing before reactor.run() ..."
    reactor.run()
    print "Nothing after reactor.run() ..."
